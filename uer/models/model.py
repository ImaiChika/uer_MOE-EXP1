...
class Model(nn.Module):
    ...
    def forward(self, src, tgt, seg):
        emb = self.embedding(src, seg)
        output, gate_scores_list = self.encoder(emb, seg)  # 从 TransformerEncoder 获取 (output, gate_scores_list)
        loss_info = self.target(output, tgt)
        return (output, gate_scores_list), loss_info  # 返回 ((output, gate_scores_list), loss_info)