...
from uer.layers.transformer1 import TransformerLayer
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, emb, seg):
        ...
        for i in range(self.layers_num):
            if self.parameter_sharing:
                hidden, gate_scores = self.transformer(hidden, mask, position_bias=position_bias)
            else:
                hidden, gate_scores = self.transformer[i](hidden, mask, position_bias=position_bias)
            gate_scores_list.append(gate_scores)  # 将每层的 gate_scores 添加到列表中

        if self.layernorm_positioning == "pre":
            return self.layer_norm(hidden)
        else:
            return hidden, gate_scores_list  # 返回隐藏状态和所有层的门控分数
