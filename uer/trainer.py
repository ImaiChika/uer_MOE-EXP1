...
class BertTrainer(Trainer):
    def __init__(self, args):
        ...
        self.total_correct_mlm = 0.0
        self.total_denominator = 0.0
        self.total_balance_loss = 0.0  # 新增：负载均衡损失统计

    def forward_propagation(self, batch, model):
        src, tgt_mlm, tgt_sp, seg = batch
        # 假设模型返回 (hidden, gate_scores_list) 和 loss_info
        (hidden, gate_scores_list), loss_info = model(src, (tgt_mlm, tgt_sp), seg)
        loss_mlm, loss_sp, correct_mlm, correct_sp, denominator = loss_info
        loss = loss_mlm / 10 + loss_sp

        # 计算负载均衡损失（仅当使用 MoE 时）
        if hasattr(self.args, 'feed_forward') and self.args.feed_forward == "moe":
            balance_loss = 0.0
            for gate_scores in gate_scores_list:  # 遍历每层的门控分数
                expert_usage = gate_scores.mean(dim=[0, 1])  # [num_experts]
                mean_usage = expert_usage.mean()
                variance = ((expert_usage - mean_usage) ** 2).mean()
                balance_loss += variance
            balance_loss /= len(gate_scores_list)  # 平均每层的均衡损失
            loss += self.args.moe_balance_weight * balance_loss  # 加到总损失中
            self.total_balance_loss += balance_loss.item()
    ...
    def report_and_reset_stats(self):
        ...
        print("| {:8d}/{:8d} steps"
            ...
            self.total_correct_mlm / self.total_denominator,
            self.total_correct_sp / self.total_instances,
            self.total_balance_loss / self.report_steps if hasattr(self.args, 'feed_forward') and self.args.feed_forward == "moe" else 0.0))

        self.total_loss, self.total_loss_mlm, self.total_loss_sp = 0.0, 0.0, 0.0
        self.total_correct_mlm, self.total_denominator = 0.0, 0.0
        self.total_correct_sp, self.total_instances = 0.0, 0.0
        self.total_balance_loss = 0.0  # 重置负载均衡损失
